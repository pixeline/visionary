...
/**
	...
	*/
	...
	var data = {},
		...
		updateOrder = function() {
			...
			if (sortable) {
				...
				$("#list-normal").removeClass("sortable").sortable({
					disabled: true
				});
		...
		checkResult = function() {
			...
			if (result.c_index <= 1.6) {
				//not colorblind
				$('.cvd_type').html("n'êtes pas daltonien");
				cvd_type = "succeed";
			} else if (result.majorAngle >= +0.7) {
				//protan color vision
				$('.cvd_type').html('êtes daltonien de type protanope');
				cvd_type = "protan";
		...
		drawDom = function() {
			...
			for (key = 0; key < colorOrder.length; key++) {
				colorListItems = $("<li></li>").data("item", colorOrder[key].id).addClass("color-item").css("background-color", colorOrder[key].color)
				$colorList.append(colorListItems);
			}

			$colorList.find("li").velocity("transition.slideDownIn", {
				duration: 400,
				stagger: 20
			})
		...
		commands = function() {
			...
			$("#choices").on("click", "a", function(e) {
				...
				switch ($(this).attr("id")) {
				case "pick-normal":
					colorOrder = orderArray(data.colors, data.normal.scheme);
					choice = "normal";
					break;
				case "pick-protan":
					colorOrder = orderArray(data.colors, data.protan.scheme);
					choice = "protan";
					break;
				case "pick-deutan":
					colorOrder = orderArray(data.colors, data.deutan.scheme);
					choice = "deutan";
					break;
				case "pick-tritan":
					colorOrder = orderArray(data.colors, data.tritan.scheme);
					choice = "tritan";
					break;
				case "pick-deuteranormal":
					colorOrder = orderArray(data.colors, data.presets.tritanCrossing);
					choice = "deutan";
					break;
				case "pick-tritan-crossing":
					colorOrder = orderArray(data.colors, data.presets.deuteranomal);
					choice = "tritan";
					break;
				case "pick-other":
		...
		drawGraph = function() {
			...
			for (key = 0; key < data.colors.length; key++) {
				...
				$textNum.text(num).css("font-size", "12px").attr("x", data.colors[key].x + offsetX).attr("y", data.colors[key].y + offsetY);
				$stage.append($textNum);

				var $circle = $(document.createElementNS("http://www.w3.org/2000/svg", "circle"));
				$circle.attr("cx", data.colors[key].x).attr("cy", data.colors[key].y).attr("r", circleRadius).attr("fill", data.colors[key].color);
				$stage.append($circle);
			}
			...
			$lineProtane.attr("x1", data.protan.from.x).attr("y1", data.protan.from.y).attr("x2", data.protan.to.x).attr("y2", data.protan.to.y).attr("stroke", "#ffaaaa").attr("stroke-width", lineThick);

			var $textProtan = $(document.createElementNS("http://www.w3.org/2000/svg", "text"));
			$textProtan.text("Protan").css({
				"font-size": "12px",
				"transform": "translate(168px, 130px) rotate(-98deg)"
			}).attr("x", 0).attr("y", 0);

			//  DEUTANE
			var $lineDeutane = $(document.createElementNS("http://www.w3.org/2000/svg", "line"));
			$lineDeutane.attr("x1", data.deutan.from.x).attr("y1", data.deutan.from.y).attr("x2", data.deutan.to.x).attr("y2", data.deutan.to.y).attr("stroke", "#aaffaa").attr("stroke-width", lineThick);

			var $textDeutane = $(document.createElementNS("http://www.w3.org/2000/svg", "text"));
			$textDeutane.text("Deutane").attr("x", 0).attr("y", 0).css({
				"font-size": "12px",
				"transform": "translate(190px, 138px) rotate(-78deg)"
			})

			//  TRITANE
			var $lineTritane = $(document.createElementNS("http://www.w3.org/2000/svg", "line"));
			$lineTritane.attr("x1", data.tritan.from.x).attr("y1", data.tritan.from.y).attr("x2", data.tritan.to.x).attr("y2", data.tritan.to.y).attr("stroke", "#aaaaff").attr("stroke-width", lineThick);

			var $textTritane = $(document.createElementNS("http://www.w3.org/2000/svg", "text"));
			$textTritane.text("Tritane").attr("x", 0).attr("y", 0).css({
				"font-size": "12px",
				"transform": "translate(194px, 160px) rotate(-7deg)"
			})
	...
	buildTest(function() {
		console.log("START")
		$("#test").removeClass("hide").velocity("transition.slideUpIn", {
			duration: 420
		});
	});
